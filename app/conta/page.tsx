...
export default async function CustomerPage() {
  ...
  const groups = await Promise.all((identities ?? []).map(async identity => {
    ...
    const { data, error: ordersError } = await supabase.from("orders").select("id,display_number,status,total_amount,created_at").eq("tenant_id", identity.tenant_id).eq("customer_id", identity.customer_id).order("created_at", { ascending: false }).limit(50);
