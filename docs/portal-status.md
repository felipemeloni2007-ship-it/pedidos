...
Em 21/09/2026 a conexão Supabase disponível não tem permissão para o projeto `cnqpecamfgtqtwxzzsnm`. Não existem credenciais locais configuradas. Portanto, migrations e testes de autenticação, isolamento e persistência no projeto remoto permanecem pendentes. Não migrar dados para outro projeto sem indicação do proprietário.
